...
def build_position_decoder(modules, Data, results_dir, nSteps):
	...
	with MOBSgraph.as_default():
		...
		for tetrode in range(n_tetrodes):
			...
			spikeEncoder                = modules['mobs_NB'].encoder(x,Data['clustersPerGroup'][tetrode],keep_proba, size=64)
			...
			with tf.variable_scope("group"+str(tetrode)+"-evaluator"):
				...
				cross_entropy       = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=ySparse, logits=spikeEncoder['Output']))
				...
				for i in range(nSteps+1):
					...
					if i%50 == 0:
						curr_eval = sess.run([MOBSgraph.get_tensor_by_name('group'+str(tetrode)+'-evaluator/accuracy:0')], 
												...
												MOBSgraph.get_tensor_by_name('group'+str(tetrode)+'-encoder/keep_proba:0'): 1.})
					...
					MOBSgraph.get_operation_by_name('group'+str(tetrode)+'-evaluator/trainer').run(
									...
									MOBSgraph.get_tensor_by_name('group'+str(tetrode)+'-encoder/keep_proba:0'): 0.5})
...
def decode_position(modules, Data, results_dir, start_time, stop_time, bin_time):
	...
	for tetrode in range(n_tetrodes):
		...
		feedDictTensors.append(       tf.get_default_graph().get_tensor_by_name("group"+str(tetrode)+"-encoder/keep_proba:0") )
