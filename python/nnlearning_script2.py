...
findFolder = lambda path: path if path[-1]=='/' or len(path)==0 else findFolder(path[:-1])
python_sys_path = [sys.argv[4], findFolder(findFolder(sys.argv[4])[:-1])+'Thibault']
with add_path(python_sys_path):
    mobsNN = __import__("mobs_nndecoding4")
    mobs_NB = __import__("mobs_networkbuilding")
    mobs_raw = __import__("rawDataParser")
modules = {"mobsNN":mobsNN, "mobs_NB":mobs_NB, "mobsRAW":mobs_raw}
...
if not os.path.isfile(projectPath.folder+'_rawSpikes.npy'):
    spikeDetector = modules['mobsRAW'].SpikeDetector(projectPath, useOpenEphysFilter)
    rawSpikes = {}
...
Data = modules['mobsNN'].build_maps(
...
position_proba, position, nSpikes, times = modules['mobsNN'].decode_position(modules, Data, NN_dir, 
