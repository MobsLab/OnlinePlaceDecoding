try isstr(position_proba);
	...
	try
		...
		Occupation=decoding_results.Occupation;
	catch
...
OccupationG=Occupation>max(max(Occupation))/15;
% try
% 	periscope_array=importdata('/home/mobshamilton/Documents/dataset/RatCatanese/periscope_array.mat');
% 	distances = P2Pdistance(OccupationG, periscope_array);
% catch
% 	disp('Did not find a periscope file :''(');
% 	distances = P2Pdistance(OccupationG);
% end
...
for n=1:size(position,2)	
	...
	%%%--- Method using maximum of probability, which is a priori wrong
	...
	%-- There are two options to compute the generalized_error (one of them requires to compute the behavioural distances)
	X_error = [X_error abs(position(1,n)-X)];
	Y_error = [Y_error abs(position(2,n)-Y)];
	generalized_error=[generalized_error norm(position([1 2],n)'-[X Y])];
	% generalized_error=[generalized_error distances(position(1,n),position(2,n),floor(X+0.5),floor(Y+0.5))];
...
% prompt = 'Do you want movie? Y/N [Y]: ';
% str = input(prompt,'s');
% if isempty(str)
%     str = 'Y';
% end
...
% 	%%%%%%%%%%%%%%%%-------------- MOVIE ---------------%%%%%%%%%%%%%%%%%%%%%%%
% 	X=X/10;  %% This seems to be necessary, don't know why, don't have to care
% 	writerObj = VideoWriter(['Movie','.avi']);
% 	writerObj.FrameRate = 0.1;
% 	open(writerObj);
% 	movie_figure = figure('visible','on');

% 	%%%--- First plot : guess of X coordinate
% 	spl3=subplot(1,9,3);
...
% 	%%%--- Second plot : guess of the Y coordinate
% 	spl2=subplot(1,9,2);
...
% 	%%%--- Third plot : norm of the standard deviation
% 	spl1=subplot(1,9,1);
...
% 		Boundaries=bwboundaries(Occupation(:,:)>max(max(Occupation))/15);
% 		for k = 1:length(Boundaries)
% 			arena=Boundaries{k};
% 			plot(arena(:,2),arena(:,1),'-g');
% 		end
% 		plot(position(2,n),position(1,n),'o','Color','r','markerfacecolor','r', 'markersize',15);hold off;
% 		% else
% 		% 	spl4=subplot(1,9,[4 5 6 7 8 9]);
...
% elseif str ~= 'n' && str ~= 'N'
% 	disp('Didn''t understand the answer, we chose to ignore it and did NOT save the movie');
% end