...
for tetrode=1:size(nb_clusters,2)
	...
	while SpikeData.s(cursor,2)==tetrode
		...
		if SpikeData.s(cursor,3)==0
			...
			features=Waveforms.W{1,sum(nb_clusters(1:tetrode-1))+find(clusters{tetrode}==SpikeData.s(cursor,3))}(clusters_cursors(SpikeData.s(cursor,3)),(1:end)',:);
...
disp('Saving ...')
save('DATA_allfeatures.mat','-v7.3');
